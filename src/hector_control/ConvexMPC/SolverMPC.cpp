...
//   A.block<3, 1>(6, 12) << cross_mat(I_inv, blockDistance)*blockForce/9.81; 
  ...
  // // centroidal augmentation matrix
  // for (s16 b = 0; b < 2; b++)
  // {
  //   B.block<3, 3>(6, b * 3) << cross_mat(Matrix<fpt, 3, 3>::Identity(), r_feet.col(b));
  // }
  // B.block<3, 3>(6, 6) << Matrix<fpt, 3, 3>::Identity();
  // B.block<3, 3>(6, 9) << Matrix<fpt, 3, 3>::Identity();  //switch
  // B.block<3, 3>(9, 0) << Matrix<fpt, 3, 3>::Identity(); //switch 
  // B.block<3, 3>(9, 3) << Matrix<fpt, 3, 3>::Identity();
  // Matrix<fpt, 13, 13> B_cmm;
  // B_cmm.setIdentity();
  // Matrix<fpt, 6, 6> A_cmm;
  // Matrix<fpt, 6, 6> A_inv;

  // A_cmm <<  0.2575,   -0.004932,   -0.02279,  0,     0,      0,     
  //           -0.004932,     0.264,   -0.01826, 0,     0,      0,    
  //           -0.02279,   -0.01826,    0.05406, 0,     0,      0,    
  //           0,           -1.518,     0.1406, 11.31, 0,      0,     
  //           1.518,            0,     -0.138, 0,     11.31,  0,     
  //          -0.1406,        0.138,      0, 0,     0,      11.31;
  // // A_cmm.block<6, 3>(0, 0) << cmm_data.cast <float> ().block(0,3,6,3);
  // // A_cmm.block<6, 3>(0, 3) << cmm_data.cast <float> ().block(0,0,6,3);
  // A_inv = A_cmm.inverse();
  // B_cmm.block<6, 6>(6, 6) << A_inv;
  // B = B_cmm * B;
